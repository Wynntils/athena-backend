...
Route::controller(CapeController::class)->group(static function () {
    ...
    Route::get('reject-edit/{token}/{sha}', 'rejectEdit')->middleware('cape.token')->name('reject-edit');
    Route::prefix('queue')->name('queue.')->group(function () {
        ...
        Route::get('approve-edit/{token}/{sha}', 'approveEdit')->middleware('cape.token')->name('approve-edit');
        Route::post('upload/{token}', 'uploadCape')->middleware('cape.token')->name('upload');
    });
});
